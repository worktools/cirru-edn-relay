...
- relay CLI 采用无状态约定：请求类命令默认连接 `ws://127.0.0.1:9100`，需要其他 relay 时显式传 `--server`
- channel 始终由每次命令上的 `--channel` 决定，不维护“当前 channel”
- `help` / `skill` 这类高层命令应通过协议查询目标 channel 上的 renderer，不要把文档硬编码在 relay CLI 里
